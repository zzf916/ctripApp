...
class HotelDetailPage(BasePage):
    ...
    def getData(self):
        ...
        if len(rooms) != 0:
            for room in rooms:
                ...
                if len(self.list_) > 0 and dict_.get('房型') in jsonpath.jsonpath(self.list_, '$..房型'):
                    continue
    ...
    def action(self):
        ...
        l2 = sorted(set(list(map(str, list1_))), key=list(map(str, list1_)).index)
        return l2
